...
contract BitUber {
  ...
  uint256 public _maxSupply = 33000000000000000000000000;
  ...
  function BitUber() public payable {
    owner = msg.sender;
    enableContract();
  }
  ...
  function transferFrom(address _from, address _to, uint256 _value) public workingFlag returns (bool success) {
    ...
    if (balances[_from] >= value
      ...
      && _value > 0
      && balances[to] + _value > balances[_to])
      {
        if ((_from == address(this))||(_to == address(this))) {
          balances[from] -= _value;
          allowed[from][_to] -= _value;
          balances[to] += _value;
          Transfer(from, _to, _value);
          return true;
        }
        else {
          if (transferAllowed2 == true) {
  ...
  function allowance(address _ownerr, address _aspender) public constant returns (uint256 remaining) {
